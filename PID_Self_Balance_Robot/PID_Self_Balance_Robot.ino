...
#define ENA 9  // Enable/speed motor A
#define IN1 14   // Direction A
#define IN2 16   // Direction A
#define ENB 6   // Enable/speed motor B
#define IN3 15   // Direction B
#define IN4 18   // Direction B
...
#define MPU6050_AXOFFSET 154
#define MPU6050_AYOFFSET -459
#define MPU6050_AZOFFSET 308
...
bool off_mot = false; // Turn Off Motors


// PID variables
double Setpoint, Input, Output;
double Kp = 10, Ki = 1, Kd = 0.5;  // PID coefficients, tune these for your robot

/*
double Kp1 = 0.25*Kp, Kp2 = 0.7*Kp, Kp3 = 0.9*Kp;
double Kd1 = 1.5*Kd, Kd2 = 1.4*Kd;
*/
...
void loop() {
  ...
  Input = roll;  // Assuming 'x' is the axis of the roll


  if (abs(Input) <= 2 )
  {Input = 0;}

  // Segmented Proportional Gain
  /*
  if (abs(roll) <= 10 )
  {Kp = Kp1;
  Kd = Kd1;
...
void driveMotors(int pwm, bool off) {

  if (pwm > 0 && !off) {

    pwm_map = map(pwm,0,255,30,255);

    analogWrite(ENA, pwm_map);
    digitalWrite(IN1, HIGH);
    digitalWrite(IN2, LOW);
    analogWrite(ENB, 1.1*pwm_map);
